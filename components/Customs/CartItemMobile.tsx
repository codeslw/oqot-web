...
export const CartItemMobile:React.FC<ICartItem> = memo(observer(({
                                                               maxCount,
                                                               photoPath,
                                                               name,
                                                               price,
    ...
    return <div className={"flex items-center w-full py-6 justify-between"}>
        <div className="flex items-center space-x-4">
            <Image width={72} height={72} src={photoPath} alt={""}/>
            <div className="flex flex-col space-y-2">
                <div
                    className={`max-w-full ${maxCount === 0 ? "text-base-light" : "text-base-light-gray"}`}>{name}</div>
        ...
        <div className="flex items-center space-x-1">
            <div
                ...
                {favouriteStore.favouriteGoods.includes(goodId) ? <HeartIconFilled className={"fill-red-default"}/> :
                    <HeartIcon className={"fill-gray-secondary hover:fill-red-default"}/>}
            </div>
            <div
